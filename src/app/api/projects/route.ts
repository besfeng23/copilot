...
export async function GET(req: Request) {
  try {
    ...
    if (err?.code === "FIREBASE_ADMIN_NOT_CONFIGURED" && Array.isArray(err?.missing)) {
      return NextResponse.json(
        {
          ok: false,
          ...
          message:
            err?.message ??
            "Firebase Admin is not configured. Set server environment variables and redeploy.",
    ...
    return NextResponse.json(
      { ok: false, code: "PROJECTS_GET_FAILED", message: "Failed to list projects." },
      { status: 500 }
    );
  }
}
