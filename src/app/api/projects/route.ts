...
export async function GET(req: Request) {
  const { requireAuth, getUserOrgRoles } = await import("@/lib/auth/server");
  ...
  try {
    const decoded = await requireAuth(req).catch((err: any) => {
      ...
      return NextResponse.json(
        { ok: false, code: "UNAUTHENTICATED", message: err?.message ?? "Not authenticated." },
        { status }
      );
    });
    ...
    if (orgRoles.length === 0) {
      // Important: do not auto-bootstrap here. The UI should offer an explicit bootstrap CTA.
      return NextResponse.json({ ok: true, orgs: [], projects: [], reason: "NO_ORG" as const });
    }
    ...
    const projects = orgs.flatMap((o) => o.projects.map((p) => ({ ...p, orgId: o.id })));
    return NextResponse.json({ ok: true, orgs, projects });
  } catch (err: any) {
    console.error("GET /api/projects failed:", err);
    return NextResponse.json(
      { ok: false, code: "INTERNAL", message: "Unexpected error." },
      { status: 500 }
    );
  }
}
